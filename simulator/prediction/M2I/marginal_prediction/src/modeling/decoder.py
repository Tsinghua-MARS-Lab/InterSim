...
import prediction.M2I.marginal_prediction.src.structs as structs
import prediction.M2I.marginal_prediction.src.utils_cython as utils_cython
from prediction.M2I.marginal_prediction.src.modeling.lib import PointSubGraph, GlobalGraphRes, CrossAttention, GlobalGraph, MLP
import prediction.M2I.marginal_prediction.src.utils as utils
import prediction.M2I.marginal_prediction.src.globals as globals
...
class Decoder(nn.Module):
    ...
    def __init__(self, args_: utils.Args, vectornet):
        ...
        self.mode_num = 50 # args.mode_num
        ...
        #     self.yaw_decoder = DecoderResCat(hidden_size, in_features=hidden_size, out_features=self.future_frame_num)
    ...
    def goals_2D_per_example_chamfer_eval(self, i: int, goals_2D: np.ndarray, mapping: List[Dict],
                                          lane_states_batch: List[Tensor],
                                          inputs: Tensor, inputs_lengths: List[int], hidden_states: Tensor,
        ...
        # get scores for all goal points
        scores = self.get_scores(goals_2D_tensor, *get_scores_inputs)
        index = torch.argmax(scores).item()
        highest_goal = goals_2D[index]

        goal_modes = 200
        # get trajectory for 200 goal points from NMS
        assert args.nms_threshold is not None, args.nms_threshold
        utils.select_goals_by_NMS(mapping[i], goals_2D, np.array(scores.tolist()), args.nms_threshold,
                                  mapping[i]['speed'], mode_num=goal_modes)

        # get trajectory for 200 goal points
        pred_goals = mapping[i]['pred_goals']
        targets_feature = self.goals_2D_mlps(torch.tensor(pred_goals, dtype=torch.float, device=device))
        hidden_attention = self.tnt_cross_attention(
            targets_feature.unsqueeze(0), inputs[i][:inputs_lengths[i]].unsqueeze(0)).squeeze(0)
        predict_trajs = self.tnt_decoder(
            torch.cat([hidden_states[i, 0, :].unsqueeze(0).expand(len(targets_feature), -1), targets_feature,
                       hidden_attention], dim=-1)).view([goal_modes, self.future_frame_num, 2])
        predict_trajs = np.array(predict_trajs.tolist())

        pred_trajs_to_return = []
        pred_traj_probs = []
        pred_traj_indices = []

        threshold = 2.0
        if len(predict_trajs.shape) != 3:
            print("Pred shape ERROR on eval: ", predict_trajs.shape)
        total_pred, pred_pts_num, _ = predict_trajs.shape

        starting_from = 50
        detect_interval = 5
        assert pred_pts_num > starting_from, predict_trajs.shape
        for j in range(total_pred):
            if len(pred_trajs_to_return) >= self.mode_num:
                break
            # check if this trajectory already in predict set
            in_predict = False
            for current_traj in pred_trajs_to_return:
                dist_per_traj = []
                for k in range(pred_pts_num):
                    if k % detect_interval != 0 or k < starting_from:
                        continue
                    # calculate the minimal distance
                    delta = predict_trajs[j][k] - current_traj
                    distance = np.sqrt(delta[:, 0] ** 2 + delta[:, 1] ** 2)
                    assert len(distance.shape) == 1, distance.shape
                    dist_per_traj.append(np.min(distance))
                if max(dist_per_traj) < threshold:
                    in_predict = True
                    break
            if not in_predict:
                pred_trajs_to_return.append(predict_trajs[j])
                pred_traj_probs.append(mapping[i]['pred_probs'][j])

        while len(pred_trajs_to_return) < self.mode_num:
            j = np.random.randint(0, total_pred)
            pred_trajs_to_return.append(predict_trajs[j])
            pred_traj_probs.append(mapping[i]['pred_probs'][j])

        if args.waymo:
            normalizer = mapping[i]['normalizer']
            for each in pred_trajs_to_return:
                each[:] = normalizer(each, reverse=True)

        assert pred_traj_probs[0] == max(pred_traj_probs), pred_traj_probs

        return pred_trajs_to_return, pred_traj_probs, None
    ...
    def forward(self, mapping: List[Dict], batch_size, lane_states_batch: List[Tensor], inputs: Tensor,
        ...
        if 'variety_loss' in args.other_params:
            ...
            if True:
                pred_trajs_per_batch = []
                pred_scores_per_batch = []

                for i in range(batch_size):
                    goals_2D = mapping[i]['goals_2D']
                    pred_traj, pred_score, _ = self.goals_2D_per_example_chamfer_eval(i, goals_2D, mapping,
                                                                                      lane_states_batch, inputs,
                                                                                      inputs_lengths,
                                                                                      hidden_states, labels,
                                                                                      labels_is_valid, device, loss, DE)
                    pred_trajs_per_batch.append(pred_traj)
                    pred_scores_per_batch.append(pred_score)
                return np.array(pred_trajs_per_batch), np.array(pred_scores_per_batch), None
