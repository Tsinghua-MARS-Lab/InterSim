...
def get_instance(args: utils.Args, inputs, decoded_example, file_name,
    ...
    if True:
        ...
        if 'direct_relation_label' in args.other_params:
            ...
            #     assert isinstance(id1, type(int(8))), id1
            #     assert isinstance(id2, type(int(8))), id2
            #
            #     direct_relation = np.array(direct_relation)
            #     if len(direct_relation.shape) == 1:
            #         direct_relation = direct_relation[np.newaxis, :]
            #     direct_relation_pair = direct_relation[:, :2].tolist()
            #     if [id1, id2] in direct_relation_pair:
            #         pair_index = direct_relation_pair.index([id1, id2])
            #         interaction_label = direct_relation[pair_index][2]
            #     elif [id2, id1] in direct_relation_pair:
            #         pair_index = direct_relation_pair.index([id2, id1])
            #         interaction_label = direct_relation[pair_index][2]
            #     else:
            #         # only train labels in the data file to get balanced labels
            #         return None
            #     # old version file
            #     # if [id2, id1] in direct_relation:
            #     #     # flip relation
            #     #     if id2 < id1:
            #     #         interaction_label = 0
            #     #     if id1 > id2:
            #     #         interaction_label = 1
            #     # elif [id1, id2] in direct_relation:
            #     #     # print("unchanged relation")
            #     #     pass
            #     # else:
            #     #     # no direct relation between two interactive agents
            #     #     interaction_label = 2
            #
            # # You can train with two additional modes
            # if 'binary_is_two' in args.other_params:
        ...
        if 'train_interest' in args.other_params and args.do_train:
            ...
            if not move_interest_objects_forward(decoded_example, sample_is_valid, gt_trajectory,
                ...
                return
        # elif 'train_pair_interest' in args.other_params and args.do_train:
        ...
        # training reactor following Influencer->Reactor
            ...
            if 'train_from_large' in args.other_params:
                ...
                if args.influencer_pred_file_path is not None:
                    ...
                    if 'draw_box' in args.other_params and 'raster_inf' in args.other_params:
                        # draw agent box on raster
                        # calculate yaw angle from trajectory
                        # TODO:
                        pass
