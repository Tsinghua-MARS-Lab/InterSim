...
import pickle
import sys
sys.path[-1] = '~/relations/reactor_git_repo/src'
...
def eval_one_epoch(model, iter_bar, optimizer, device, args: utils.Args, i_epoch,
    ...
    for step, batch in enumerate(iter_bar):
        ...
        if 'save_rst' in args.other_params:
            ...
            if 'train_reactor' in args.other_params:
                if 'pred_yaw' in args.other_params:
                    assert pred_trajectory.shape[0] == pred_yaw.shape[0], f'{pred_trajectory.shape} / {pred_yaw.shape}'
        ...
        if 'train_relation' not in args.other_params:
            if 'pred_yaw' not in args.other_params:
                motion_metrics.args = args

                # Pad 0s if predicted trajectory is not complete.
                if pred_trajectory.shape[2] < 80:
                    pred_trajectory_padding = np.zeros((pred_trajectory.shape[0],
                                                        pred_trajectory.shape[1],
                                                        80 - pred_trajectory.shape[2],
                                                        pred_trajectory.shape[3]))
                    pred_trajectory = np.concatenate([pred_trajectory, pred_trajectory_padding], 2)
                ...
                motion_metrics.args = args
                # TODO(cyrushx): Fix dim of pred_trajectory when it is incomplete.
                motion_metrics.update_state(
        ...
        if 'pred_yaw' in args.other_params:
            total_agent_num, k, frame_number = pred_yaw.shape
            loss_per_batch = 0
            for y, each_agent_yaw in enumerate(pred_yaw):
                # calculate and print metrics only for yaw prediction
                gt_yaw_per_agent = batch[y]['yaw_labels']
                pred_yaw_per_agent = pred_yaw[y]
                loss_per_batch += np.sum(abs(gt_yaw_per_agent - pred_yaw_per_agent)) / frame_number / k
            total_yaw_loss += loss_per_batch / total_agent_num
    ...
    if 'pred_yaw' in args.other_params:
        print("Yaw pred eval:", total_yaw_loss)
...
def train_one_epoch(model, iter_bar, optimizer, device, args: utils.Args, i_epoch, queue=None, optimizer_2=None):
    ...
    #             a_li_ADE, a_li_FDE = queue.get()
    #             li_ADE.extend(a_li_ADE)
    #             li_FDE.extend(a_li_FDE)
    #         assert queue.empty()
