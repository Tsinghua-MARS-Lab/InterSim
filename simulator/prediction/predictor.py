    ...
    def __call__(self, data):
        ...
        for id in data['agent']:
            ...
            if self.all_agents or to_predict:
                ...
                pred_yaw = np.zeros(80)
                for i in range(80):
                    if i > 0:
                        x, y = pred_trajectory[i - 1, 0], pred_trajectory[i - 1, 1]
                    else:
                        x, y = pose[10, 0], pose[10, 1]
                    pred_yaw[i] = get_angle(pred_trajectory[i, 0] - x, pred_trajectory[i, 1] - y)
                    if pred_yaw[i] < 0:
                        pred_yaw[i] += 2.0 * math.pi
                    if abs(delta_x) + abs(delta_y) < 0.05:
                        pred_yaw[i] = pose[10, -1]

                agent_to_pred[id] = {}
                agent_to_pred[id]['pred_trajectory'] = pred_trajectory
                agent_to_pred[id]['pred_yaw'] = pred_yaw
...
class PredictionLoaderMultiTime:
    r"""
            ...
            if self.all_agents or to_predict:
                ...
                if scenario in prediction_loaded:
                    if agent_id in prediction_loaded[scenario]:
                        ...
                        if 'rst' not in prediction_loaded[scenario][agent_id]:
                            # load with offset
                            loaded_agent_pred = prediction_loaded[scenario][agent_id]
                            total_time_offsets = len(list(loaded_agent_pred.keys()))
                            agent_to_pred[agent_id] = {}
                            for i, time_offset in enumerate(loaded_agent_pred):
                                agent_to_pred[agent_id][time_offset] = {}
                                pred_trajectory = np.zeros([6, 80, 2])
                                assert len(pose) >= 11, len(pose)
                                x, y = pose[10, 0], pose[10, 1]
                                pred_yaw = np.zeros([6, 80])
                                ...
                                pred_scores = np.exp(loaded_agent_pred[time_offset]['score'])
                                loaded_pred = loaded_agent_pred[time_offset]['rst']

                                for each_prediction in range(6):
                                    pred_trajectory[each_prediction, :, :] = loaded_pred[each_prediction, :, :]
                                    for i in range(80):
                                        if i > 0:
                                            x, y = pred_trajectory[each_prediction, i - 1, 0], pred_trajectory[
                                                each_prediction, i - 1, 1]
                                        else:
                                            x, y = pose[10, 0], pose[10, 1]
                                        pred_yaw[each_prediction, i] = get_angle(pred_trajectory[each_prediction, i, 0] - x,
                                                                                 pred_trajectory[each_prediction, i, 1] - y)
                                ...
                                agent_to_pred[agent_id][time_offset]['pred_trajectory'] = pred_trajectory
                                agent_to_pred[agent_id][time_offset]['pred_yaw'] = pred_yaw
                                if np.sum(pred_scores) > 0.01:
                                    agent_to_pred[agent_id][time_offset]['pred_scores'] = pred_scores / np.sum(pred_scores)
                                else:
                                    agent_to_pred[agent_id][time_offset]['pred_scores'] = pred_scores
                        else:
                            print(list(prediction_loaded[scenario][agent_id].keys()))
                            assert False, f'rst in prediction result, is it without time offset? if so, use the predictor without time offset'

                else:
                    # skip scenarios not in prediction result file
                    print(f'scenario {scenario} not found in prediction result')
                    return None
