...
class DummyPlanner(EnvPlanner):
    ...
    def plan_ego(self, current_state, current_frame_idx):
        ...
        if frame_diff >= 0 and frame_diff % self.planning_interval == 0:
            ...
            my_current_a_per_step = my_current_v_per_step - euclidean_distance(
                current_state['agent'][ego_agent_id]['pose'][current_frame_idx - 2, :2],
                current_state['agent'][ego_agent_id]['pose'][current_frame_idx - 7, :2]) / 5
            ...
            vehicle_state = [my_current_pose[0], my_current_pose[1],  # x, y
                             my_current_pose[3],  # yaw
                             my_current_v_per_step * 10, 0,  # v_lon, vlat
                             my_current_a_per_step * 100,  # v_long_dot
                             0, 0]
            rst = car.vehicle_dyn(veh_state=vehicle_state, actions_num=[1, 0.1], frequency=0.1, length=8)
            print("test current v: ", my_current_v_per_step* 10, my_current_a_per_step*100, my_current_pose[3])
            x_list, y_list, theta_list, _, _, _, _, _ = rst
            planed_traj = np.zeros([80, 4])
            planed_traj[:, 0] = x_list
            planed_traj[:, 1] = y_list
            planed_traj[:, 3] = theta_list
