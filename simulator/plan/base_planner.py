                    ...
                    for n in range(k):
                        ...
                        for t in range(total_frames_in_pred):
                            if t == total_frames_in_pred - 1:
                                ...
                                pred_traj_with_yaw[t, 3] = utils.get_angle_of_a_line(pt1=pred_traj[t, :2],
                                                                                     pt2=pred_traj[t + 1, :2])
            ...
            for each_agent in current_state['agent']:
                ...
                if int(current_state['agent'][each_agent]['type']) not in self.vehicle_types:
                    ...
                    steady_in_past = euclidean_distance(
                        each_agent_pose[current_frame_idx - 1, :2],
                        each_agent_pose[current_frame_idx - self.frame_rate, :2]) < 3
    ...
    def plan_ego(self, current_state, current_frame_idx):
        ...
        if self.is_planning(current_frame_idx) and current_state is not None:
            ...
            if ego_to_yield:
                ...
                current_state['predicting']['trajectory_to_mark'].append(my_interpolated_marginal_trajectories[index_to_select])
            ...
            if ego_to_yield:
                ...
                elif my_current_v_per_step < 1 / self.frame_rate and euclidean_distance(my_traj[0, :2], my_traj[-1, :2]) < MINIMAL_DISTANCE_TO_TRAVEL:
                    planed_traj = self.get_trajectory_from_interpolator(my_interpolator=my_interpolator,
                                                                        my_current_speed=my_current_v_per_step,
                                                                        desired_speed=my_target_speed,
                                                                        hold_still=True)
            ...
            if total_time_frame - current_frame_idx > 1 and len(planned_shape) > 1 and planned_shape[1] == 4:
                current_state['agent'][ego_agent_id]['pose'][current_frame_idx:planning_rst_horizon + current_frame_idx, :] = planed_traj[:total_time_frame - current_frame_idx, :]
                current_state['agent'][ego_agent_id]['pose'][planning_rst_horizon + current_frame_idx:, :] = -1
            else:
